...
def energy_economy_pipeline():
    ...
    # Path staging di server — semua task pakai path ini
    STAGING = "/home/inter24/dags/energy_dwh/staging"
    ...
    def extract_worldbank() -> str:
        ...
        for fname, indicator in files.items():
            ...
            if os.path.exists(fpath):
                ...
                # Pastikan kolom value bernama sesuai indicator
                # Struktur file: period_raw, country, {indicator}
        ...
        if dfs:
            # Gabungkan semua ke satu file staging
    ...
    def transform(
        ...
        os.makedirs(f"{STAGING}/clean", exist_ok=True)

        # Helper normalisasi period
        ...
        print(f"Kolom: {df_retail.columns.tolist()}")
        ...
        if not df_retail.empty:
            ...
            out_retail = f"{STAGING}/clean/fact_retail.csv"
        ...
        print(f"Kolom: {df_gen.columns.tolist()}")

        if not df_gen.empty:
            df_gen.columns = [c.lower().strip() for c in df_gen.columns]

            # Handle nama kolom fueltypeid yang mungkin berbeda
            fuel_col = next(
                (c for c in df_gen.columns if "fuel" in c and "id" in c), None
            )
            ...
            out_gen = f"{STAGING}/clean/fact_generation.csv"
        ...
        if wb_path and os.path.exists(wb_path):
            ...
            print(f"World Bank raw: {df_wb.shape}")
            print(f"Kolom: {df_wb.columns.tolist()}")

            # Normalisasi period_raw → period
            ...
            # Konversi semua kolom nilai ke numerik
            indicator_cols = [
                c for c in df_wb.columns
                if c not in ["period_raw", "period", "country"]
            ]
            for col in indicator_cols:
                df_wb[col] = pd.to_numeric(df_wb[col], errors="coerce")

            df_wb["period_dt"] = pd.to_datetime(
                df_wb["period"], format="%Y-%m", errors="coerce"
            )
            ...
            df_wb = df_wb.drop_duplicates(
                subset=["period", "country"]
            ).reset_index(drop=True)

            out_wb = f"{STAGING}/clean/worldbank_combined.csv"
            df_wb.to_csv(out_wb, index=False)
            print(f" World Bank clean: {df_wb.shape} → {out_wb}")
            print(f"  Kolom: {df_wb.columns.tolist()}")
            print(f"  Negara: {sorted(df_wb['country'].unique())}")
        else:
    ...
    def load(summary: dict) -> None:
        ...
        if not DB_URL:
            print(" SUPABASE_DB_URL tidak ditemukan")
            print("Jalankan: export SUPABASE_DB_URL='postgresql://...'")
            return
